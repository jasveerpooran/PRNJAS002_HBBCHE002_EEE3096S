...
int delayTime =500; // sets inital delay to 500ms
int currentMillis = 0;
int prevMillis =0;
uint32_t crr_val =0;
int dutyCycle=0;
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  ans = pollADC(); // this will start, poll and stop the ADC and return the most recent ADC value, since ADC in 12 bit mode
	                   // only values between 0-4096 will be output
	  sprintf(buffer,"ADC value is: %u \r\n",ans);
	  ...
	  sprintf(buffer2,"Duty Cycle: %d \r\n",dutyCycle);
	  HAL_UART_Transmit(&huart2, buffer2, sizeof(buffer2), 1000);
	  HAL_Delay(delayTime); // wait for delayTime
    /* USER CODE END WHILE */
...
//when button pressed we enter this method, delayTime is initially 500ms
	...
	if((currentMillis-prevMillis >200)&& (B1_Pin == 1)){
...
uint32_t pollADC(void){
	...
	HAL_ADC_Start(&hadc); //ADC start value
	HAL_ADC_PollForConversion(&hadc,100); //ADC polled
	uint32_t val = HAL_ADC_GetValue(&hadc); //ADC value obtained and stored in val
	HAL_ADC_Stop(&hadc); // ADC stopped
	return val;
}
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	uint32_t val = (47999/4096) * adc_val; // ARRmax/ADCmax * current ADC val
	return val;
}
